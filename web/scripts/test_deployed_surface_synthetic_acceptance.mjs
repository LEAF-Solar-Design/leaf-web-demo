...
function identifierHash(config, value) {
  return createHash('sha256').update(`${config.runId}\0${value}`).digest('hex')
}
...
function fakeDependencies({
  ...
  return {
    ...
    browserImpl: async (config, results) => results.map((result) => ({
      ...
      browser_bearer_hash: identifierHash(
        config,
        config.tenants[result.label === 'A' ? 0 : 1].jwt,
      ),
...
function fakeChromium(
  ...
  results,
  {
    extraOrigin = null,
    finalPath = '/try',
    finalOrigin = config.targetUrl,
    protectedOrigin = config.targetUrl,
    protectedStatus = 200,
    tokenIndices = [0, 1],
  } = {},
) {
        ...
        async newContext(contextOptions) {
          ...
          return {
            async addInitScript(script, payload) {
              ...
              if (tokenIndex === null || tokenIndex === undefined) local.delete('leaf.jwt')
              else local.set('leaf.jwt', config.tenants[tokenIndex].jwt)
            ...
            async newPage() {
              ...
              return {
                ...
                async evaluate(script, payload) {
                  const priorWindow = globalThis.window
                  const priorFetch = globalThis.fetch
                  const protectedFetch = async (path, options) => {
                    const authorization = String(options?.headers?.authorization || '')
                    const bearer = authorization.startsWith('Bearer ')
                      ? authorization.slice('Bearer '.length)
                      : ''
                    state.protectedRequests.push({
                      path,
                      method: options?.method,
                      credentials: options?.credentials,
                      cache: options?.cache,
                      redirect: options?.redirect,
                      bearerIndex: config.tenants.findIndex((tenant) => tenant.jwt === bearer),
                    })
                    return {
                      status: protectedStatus,
                      ok: protectedStatus === 200,
                      url: `${protectedOrigin}${path}`,
                    }
                  }
                  globalThis.window = {
                  ...
                  globalThis.fetch = protectedFetch
                  try {
                    return await script(payload)
                  } finally {
                    if (priorWindow === undefined) delete globalThis.window
                    else globalThis.window = priorWindow
...
describe('surface synthetic browser contract', () => {
  ...
  it('primes each isolated browser with its Auth0 token and intended drawing', async () => {
    ...
    assert.deepEqual(fake.state.protectedRequests, [
      {
        path: '/api/deployment-identity',
        method: 'GET',
        credentials: 'omit',
        cache: 'no-store',
        redirect: 'error',
    ...
    assert.deepEqual(
      observed.map((entry) => entry.protected_api_authenticated),
      [true, true],
    )
    assert.ok(observed.every((entry) => /^[a-f0-9]{64}$/.test(entry.browser_bearer_hash)))
    assert.notEqual(observed[0].browser_bearer_hash, observed[1].browser_bearer_hash)
    assert.deepEqual(observed.map((entry) => entry.label), ['a', 'b'])
  })

  it('rejects a bare browser context without an assigned bearer', async () => {
    const { config, results } = browserFixture()
    const fake = fakeChromium(config, results, { tokenIndices: [null, 1] })
    await assert.rejects(
      () => runRealBrowser(config, results, fake.chromium),
      /exact protected same-origin API with its assigned bearer/,
    )
    assert.deepEqual(fake.state.protectedRequests, [])
  ...
  it('rejects a protected API refusal and a swapped valid principal token', async () => {
    const { config, results } = browserFixture()
    const refused = fakeChromium(config, results, { protectedStatus: 401 })
    await assert.rejects(
      () => runRealBrowser(config, results, refused.chromium),
      /exact protected same-origin API with its assigned bearer/,
    )
    assert.equal(refused.state.protectedRequests[0].bearerIndex, 0)

    const swapped = fakeChromium(config, results, { tokenIndices: [1, 0] })
    await assert.rejects(
      () => runRealBrowser(config, results, swapped.chromium),
      /exact protected same-origin API with its assigned bearer/,
    )
    assert.equal(swapped.state.protectedRequests[0].bearerIndex, 1)
  })

  it('rejects a protected response attributed to another origin', async () => {
    const { config, results } = browserFixture()
    const fake = fakeChromium(config, results, { protectedOrigin: 'https://example.test' })
    await assert.rejects(
      () => runRealBrowser(config, results, fake.chromium),
      /exact protected same-origin API with its assigned bearer/,
    )
...
describe('surface synthetic artifacts', () => {
  it('publishes hashes only and keeps exact required assertion order', () => {
    ...
    assert.deepEqual(Object.keys(proof.browser[0]).sort(), [
      'backend_ready',
      'drawing_hash',
      'label',
      'live_services',
