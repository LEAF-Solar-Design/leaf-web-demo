...
function Wait-Json([string]$Url, [scriptblock]$Accept, [int]$Seconds = 120) {
...
try {
  ...
  Wait-Json "http://127.0.0.1:$AppPort/api/health" { param($x) $x.ok -and -not $x.aps_live } | Out-Null
  ...
  try {
    if ($Spec) { npx playwright test $Spec --config playwright.local.config.mjs }
    else { npm run proof:local }
