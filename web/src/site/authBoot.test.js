...
describe('console boot back-compat', () => {
  ...
  it('keeps /try on the stage for the surface-scoped params', () => {
    // /try?demo=1 is the one exception, pinned under 'one-shell demo entry'.
...
describe('one-shell demo entry', () => {
  it('boots the forwarded /try?demo=1 into the cockpit demo', () => {
    expect(bootWantsApp('?demo=1', '/try')).toBe(true)
    // The landing's own Try button target.
    expect(bootWantsApp('?demo=1', '/app')).toBe(true)
  })

  it('only the literal demo=1 on /try counts', () => {
    expect(bootWantsApp('', '/try')).toBe(false)
    expect(bootWantsApp('?demo=0', '/try')).toBe(false)
    expect(bootWantsApp('?demo=tour', '/try')).toBe(false)
    expect(bootWantsApp('?ops=1', '/try')).toBe(false)
    expect(bootWantsApp(CALLBACK, '/try')).toBe(false)
  })
