...
export default function useDrawingVersionController({
  ...
  const runVersionMutation = useCallback(async (operation, adapter, event) => {
    ...
    try {
      const view = await adapter(drawingId)
  ...
  const undo = useCallback(
    () => runVersionMutation('undo', undoVersion, 'undo'),
    [runVersionMutation, undoVersion],
  )

  const redo = useCallback(
    () => runVersionMutation('redo', redoVersion, 'redo'),
