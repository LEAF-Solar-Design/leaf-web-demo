...
function refused(): never { throw new Error("tenant Forge configuration refused"); }
...
export function protectedJson(path: string, maximum: number): unknown {
...
export function createTenantForgeConfigurationFromEnv(
  ...
  if (path === undefined && credentialDir === undefined && mode === undefined) return undefined;
  if ((mode !== undefined && mode !== "forgejo") || !path || !credentialDir
    || !isAbsolute(path) || !isAbsolute(credentialDir)) refused();
  ...
  return {
    ...
    locator: {
      repoRef: async tenant => {
        const selected = entry(tenant);
        if (selected.mode === "forge") return selected.remoteUrl;
        if (!localRepoRef) refused();
        return localRepoRef(tenant);
      },
    },
    isRemoteTenant: tenant => entry(tenant).mode === "forge",
  };
}
