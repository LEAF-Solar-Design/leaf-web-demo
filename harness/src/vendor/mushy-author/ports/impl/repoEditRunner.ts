...
export class SdkRepoEditor implements RepoEditor {
  ...
  async edit(input: RepoEditInput): Promise<RepoEditResult> {
    const sdk = (await dynImport(["@anthropic-ai", "claude-agent-sdk"])) as SdkModule;
    const { z } = (await dynImport(["zod"])) as { z: Record<string, (...a: never[]) => unknown> & { enum(v: readonly string[]): unknown; string(): unknown } };
        ...
        options: {
          ...
          canUseTool: async (toolName: string, inp: Record<string, unknown>) =>
            allowed.has(toolName)
              ...
              : { behavior: "deny" as const, message: `only the repo_* tools are permitted in the in-app edit session (got ${toolName})` },
        },
      });
      let turns = 0;
