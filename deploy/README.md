...
1. **Build (this repo)**: `.github/workflows/build-platform-images.yml` runs on
   every push to `main` that touches a build input and builds app, broker,
   canonical-worker, harness, and web from one full application commit.
   Docs-only pushes (every changed file matches `**.md` or `docs/**`) skip the
   run entirely; with no build run there is no `workflow_run` event, so the
   staging relay never fires and staging stays on the previous build. The
   filter fails open: if GitHub cannot compute the push diff, the build runs.
   Such a skipped commit has no `prod-<shortsha>` supply set of its own — the
   promote path is unaffected because it pins an explicit earlier build run
   ID and only requires its source to be an ancestor of `main`, not the tip. The canonical-worker build also checks
