...
| `LEAF_SESSION_ANNEX_STORE` | app | `legacy` | authority selector for `session_checkpoints` and `session_policies`, which 0012 left on SQLite. Separate from `LEAF_SESSIONS_STORE` because the two authorities have different prerequisites (this annex has no backfill, no parity command and no reverse writer) and incompatible rollback paths, so each cutover has to be decided on its own. **It does NOT defer the schema:** `migration_manifest()` globs every shipped `.sql` and the readiness check is selector-blind, so `0029` must be applied BEFORE this image reaches any PostgreSQL-connected deployment regardless of this value — staging sets `LEAF_PLATFORM_POSTGRES_REQUIRED=1`, so the startup assertion fails closed and the app does not start. **`LEAF_SESSIONS_STORE=postgres` requires this to be `postgres` too** and the app refuses to start otherwise: every other annex mode still READS the SQLite file at `SESSIONS_DB`, so the session would outlive its own checkpoints |
...
| `LEAF_CHECKOUT_CAP_SECRET` | app | empty | signing secret for the drawing-checkout capability (the proof of single-writer ownership). REQUIRED when `LEAF_RUNTIME_ENV=production`, with at least 32 bytes and about 128 bits of estimated entropy. Generate it with `openssl rand -hex 32`; repeated characters, words, or short pasted blocks are refused. The app refuses to mint or verify without a strong shared secret because a weak or per-process fallback could not safely prove ownership across callers, replicas, or restarts. Off production, unset means a per-process random secret and checkouts do not survive an app restart. |
