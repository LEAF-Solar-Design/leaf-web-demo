...
- [ ] Carry the trusted validated source through the harness-to-broker test request.
- [ ] Execute that source only on `aps_live=false` inside the configured sandbox and bind its receipt.
- [ ] Prove live APS refusal, staged-source execution, stale published-body exclusion, dry-run safety, and source-hash binding.
