...
def test_the_dispatch_stages_configured_services_on_the_prewarm_rail():
...
def test_web_only_is_staged_because_no_consumer_adopts_the_app_stage():
    """Replace both-service freshness with web-only staging and measured adoption evidence."""
    ...
    assert "digest intersection zero" in comment
    assert "STEP=plan-live-leg" in comment
    assert "reap-leaf-platform-staging-prewarm-stages.yml" in comment
    assert "restore `web app` when the native release consumer prefers" in comment
    assert "tree equals the admitted release's tree" in comment
    assert "native descale returns the" in comment
    assert "idle colour to 0 between groups" in comment
...
def test_the_group_workflow_has_only_the_dispatch_trigger():
    ...
    for key, value in document["env"].items():
        if key == "STAGE_SERVICES":
            # Previously equal at web app; only the group now stages web.
            assert value == "web"
...
def test_codebuild_dispatch_and_relay_receipt_executed(tmp_path, response, disposition, service, expected_present):
    ...
    # Previously both dispatched; now execute web presence and app absence.
...
def test_s3_receipt_put_metadata_and_best_effort_upload_executed(tmp_path):
    ...
    # Previously app and web; the uploaded group receipt now configures web only.
    assert receipt["configured_services"] == ["web"]
    assert receipt["configured_services"] == sorted(set(group_workflow_document()["env"]["STAGE_SERVICES"].split()))
