...
- Entities are NEUTRAL: a frame polyline carries its cell as `row` and `col`, a pile
  carries its committed values under `pile` (placement, source frame, template,
  diameter, reveal, embedment, depth in metres). How the plugin encodes those in a
  drawing is not reproduced here; the adapter that reads that encoding lives with the
  plugin, outside this public repository.
...
# --- layer names (LayerNames.cs:36,53,104; LeafCollisionCommand.cs:29-30;
# LeafPilingCommand.cs:36-38) ----------------------------------------------------
...
SHORT_MAX = 32767  # a frame's committed row and column saturate here (LeafGenerateCommand.cs:605-614)
...
def _read_frame_cell(ent):
    """The frame-cell read, LeafPilingCommand.cs:1115-1138 (same loop as
    LeafCollisionCommand.cs:110-125), neutral: the frame's committed `row` and `col`.
    None when the polyline carries no cell (it is not a generated frame)."""
    row, col = ent.get("row"), ent.get("col")
    if row is None and col is None:
        return None
    for value in (row, col):
        if isinstance(value, bool) or not isinstance(value, int):
            raise GroundFramesError("frame_cell_malformed", "a frame's row and col must be integers")
    return row, col
...
def read_tracker_frames(entities):
    ...
    vertices and a frame cell (row, col). Entity order is kept."""
    frames = []
...
def frame_entities(frames, color_index, meters_per_unit, terrain_z_m=None, site_revision=None):
    ...
    centroid in drawing units, 0.0 without terrain or off the grid. Each carries its
    committed cell (row and col, saturating at 32767, :605-614) and the site revision
    it was drawn under (None when none is set)."""
    if not frames:
        return []
    mpu = meters_per_unit if meters_per_unit > 0.0 else 1.0
    color = frame_color(color_index)
    revision = str(site_revision) if site_revision is not None and str(site_revision).strip() else None
    out = []
    for f in frames:
        elevation = 0.0
        ...
        out.append({"type": "LWPOLYLINE", "layer": LAYER_TRACKERS, "closed": True,
                    ...
                    "elevation": elevation, "row": min(f["row"], SHORT_MAX), "col": min(f["col"], SHORT_MAX),
...
def run_collision(entities):
    ...
    only when there is at least one overlap."""
    frames = read_tracker_frames(entities)
    if not frames:
        return {"status": "no_frames", "frame_count": 0, "collisions": [], "markers": []}
...
def pile_record(pile, diameter, reveal, embed, depth_m):
    """The values DrawPiles commits with each pile, LeafPilingCommand.cs:1358-1392, as
    a neutral record: placement ("Grid", "Joint" with its joint kind, or "Station"
    with its station kind), the source frame, template, mapping and station label
    (None when blank, as the plugin omits them), and the four sizes in metres."""
    if pile["placement_mode"] == "AxisStations":
        placement, detail = "Station", str(pile["station_kind"])
    elif pile["is_joint_pile"]:
        placement, detail = "Joint", str(pile["joint_kind"])
    else:
    ...
    return {"placement": placement, "placement_detail": detail,
            "source_tracker": text(pile["source_tracker_handle"]), "template": text(pile["template_name"]),
            "mapping": text(pile["mapping_signature"]), "station_label": text(pile["station_label"]),
            "diameter_m": diameter, "reveal_m": reveal, "embedment_m": embed, "depth_m": depth_m}
...
def pile_entities(piles, cfg, meters_per_unit):
    ...
    for p in piles:
        ...
        out.append({"type": "CIRCLE", "layer": LAYER_PILING, "color": dict(BY_LAYER),
                    ...
                    "thickness": depth_du, "pile": pile_record(p, diameter, reveal, embed, depth_m),
...
def read_pile_depth_m(ent):
    """TryReadPileDepthM, LeafCollisionRangeCommand.cs:166-191, neutral: the pile's
    committed depth in metres; absent, non-numeric or <= 0 means no value."""
    record = ent.get("pile")
...
def run_pile_range_check(entities, preset, meters_per_unit):
    ...
    length is its committed depth, else its Z extent times meters_per_unit."""
    ents = _entities(entities)
