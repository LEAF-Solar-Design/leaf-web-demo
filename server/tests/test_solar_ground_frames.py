...
def tracker_entity(verts, row, col, handle=None, layer="LEAF-TRACKERS"):
    ent = {"type": "LWPOLYLINE", "layer": layer, "closed": True, "vertices": verts,
           "xdata": [(1001, "LEAFFRAMECELL"), (1070, row), (1070, col)]}
    if handle:
        ent["handle"] = handle
    return ent
...
def test_parse_pvcase_piling_json_summit_lake_xdata_blob_places_two_piles():
...
def test_generate_canonical_fixture_draws_108_frame_polylines_with_framecell_xdata():
    ...
    assert first["xdata"] == [(1001, "LEAFFRAMECELL"), (1070, 0), (1070, 0)]
    assert last["xdata"] == [(1001, "LEAFFRAMECELL"), (1070, 5), (1070, 17)]
    assert last["vertices"][2] == pytest.approx((18 * 5.272, 6 * 28.828))
    assert [(e["xdata"][1][1], e["xdata"][2][1]) for e in ents[:19]] == [(0, c) for c in range(18)] + [(1, 0)]
    assert result["layers_created"] == [{"name": "LEAF-TRACKERS", "aci": 3}]
    assert result["regapps"] == ["LEAFFRAMECELL"]


def test_generate_site_revision_and_colour_rules():
    ents = canonical_frames(site_revision="rev-7")["entities"]
    assert ents[0]["xdata"][3] == (1000, "rev-7")
    ...
    assert gf.frame_xdata(40000, 3) == [(1001, "LEAFFRAMECELL"), (1070, 32767), (1070, 3)]
...
def test_generate_shading_restriction_and_road_buffers_exclude_frames():
    ...
    assert result["frame_count"] < 108
    assert result["entities"][0]["xdata"][1:] != [(1070, 0), (1070, 0)]
...
def test_generate_multi_restarts_indexes_per_boundary():
    ...
    assert result["entities"][108]["xdata"][1:] == [(1070, 0), (1070, 0)]
...
def test_collision_marks_a_nudged_frame_on_leaf_collision():
    ...
    assert result["layers_created"] == [{"name": "LEAF-COLLISION", "aci": 1}]
...
def test_collision_reads_only_valid_tracker_frames():
    ents = [tracker_entity(rectangle(5, 5), 0, 0),
            ...
            dict(tracker_entity(rectangle(5, 5), 0, 3), xdata=[(1001, "LEAFFRAMECELL"), (1070, 1)]),
            dict(tracker_entity(rectangle(5, 5), 0, 4), vertices=[(0, 0), (5, 0), (5, 5)]),
            dict(tracker_entity(rectangle(5, 5), 0, 5), xdata=[(1001, "OTHER"), (1070, 1), (1070, 2)])]
...
def test_piling_canonical_fixture_with_default_templates():
    ...
    assert first["xdata"] == [(1001, "LEAFPILING"), (1000, "Grid"), (1000, ""), (1000, "SourceTracker=200"),
                              (1000, "Template=Full"), (1000, ""), (1000, ""), (1000, "DiameterM=0.15"),
                              (1000, "RevealM=0.5"), (1000, "EmbedmentM=1.5"), (1000, "DepthM=2")]
...
def test_piling_with_terrain_drapes_in_drawing_units_and_erases_old_piles():
    ...
    assert ent["xdata"][3] == (1000, "SourceTracker=2A")
...
def test_piling_places_joint_piles_from_the_preset_tracker_pack():
    ...
    assert result["entities"][2]["xdata"][1:3] == [(1000, "Joint"), (1000, "JointGap")]
...
def test_piling_template_overrides_road_clip_and_native_mapping():
    ...
    assert result["entities"][0]["xdata"][7:] == [(1000, "DiameterM=0.2"), (1000, "RevealM=1"),
                                                  (1000, "EmbedmentM=2.5"), (1000, "DepthM=3.5")]
    ...
    assert mapped["entities"][0]["xdata"][4:6] == [(1000, "Template=Mapped"),
                                                   (1000, "Mapping=NATIVE|LEAF-TRACKERS|ACTIVE-PRESET")]
...
def test_format_xdata_double_matches_dotnet_custom_format():
    assert [gf.format_xdata_double(v) for v in (2.0, 0.15, 1.23456789012, 0.123456785, 1e-10, -0.0, 3.5)] == [
        "2", "0.15", "1.23456789", "0.12345679", "0", "0", "3.5"]
...
def test_range_check_falls_back_to_z_extent_and_rejects_bad_windows():
    ...
    stretched = {"type": "CIRCLE", "layer": "LEAF-PILING", "center": (0.0, 0.0, -10.0), "radius": 1.0,
                 "thickness": 10.0, "xdata": [(1001, "LEAFPILING"), (1000, "DepthM=abc")]}
...
def test_read_pile_depth_first_depth_item_decides():
    ent = {"xdata": [(1001, "LEAFPILING"), (1000, "Grid"), (1000, "depthm=4.25"), (1000, "DepthM=9")]}
    assert gf.read_pile_depth_m(ent) == 4.25
    assert gf.read_pile_depth_m({"xdata": [(1001, "LEAFPILING"), (1000, "DepthM=-1"), (1000, "DepthM=9")]}) is None
    assert gf.read_pile_depth_m({"xdata": [(1001, "OTHER"), (1000, "DepthM=9")]}) is None
...
def test_entity_and_xdata_refusals():
    refusal("entities_malformed", gf.run_collision, {"not": "a list"})
    refusal("entities_malformed", gf.run_collision, [42])
    bad = tracker_entity(rectangle(5, 5), 0, 0)
    bad["xdata"] = [(1001, "LEAFFRAMECELL"), (1070, "zero"), (1070, 1)]
    refusal("xdata_malformed", gf.run_collision, [bad])
    bad["xdata"] = [(1001,)]
    refusal("xdata_malformed", gf.run_collision, [bad])
