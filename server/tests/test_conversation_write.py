...
KNOWN GAP (see conversations.py's "message write path" section docstring):
create_message targets a `conversation_messages` table that has no migration
in this repo yet -- adding one is outside this card's file budget (migration:
false; files_expected names no migration file) and would break the currently
green test_postgres_authority_inventory_contract.py's pinned
EXPECTED_MIGRATIONS. Every test below that would exercise create_message
against a real database is marked requires_database; none of them can pass
until that companion migration lands, and they are expected to skip (not
run, not pass) in this environment.
"""
...
def test_idempotent_replay_returns_stored_result_with_no_duplicate_row(pg) -> None:
    ...
    first, first_inserted = conversations.create_message(
        org_id, project_id, conversation_id, binding_id,
        "hello", {"role": "user"}, "replay-key-1",
    )
    assert first_inserted is True
    second, second_inserted = conversations.create_message(
        org_id, project_id, conversation_id, binding_id,
        "hello", {"role": "user"}, "replay-key-1",
    )
...
def test_different_conversations_reuse_the_same_idempotency_key_independently(
    ...
    message_a, inserted_a = conversations.create_message(
        org_id, project_id, conv_a["conversation_id"], binding_id,
        "a", {}, "same-key",
    )
    message_b, inserted_b = conversations.create_message(
...
@requires_database
def test_write_is_two_data_statements_authority_plus_insert(pg, monkeypatch) -> None:
    ...
    assert replay_statement_count == 1, statements
