...
def test_1_run_returns_202_fast(stack):
    ...
    This branch additionally restores the documented 200ms submit bound as a
    THIRD assertion, on the argument that the catalog GET (now built outside the
    timed window, above) was the cause of the old flake. That argument is not
    settled by the numbers in this docstring: the 210ms-to-1720ms spread was
    attributed to scheduling noise, which removing the GET does not remove. The
    bound is kept here so review can decide it against a real run, and it is the
    first assertion to drop if it flakes -- the two bounds above still DECIDE
    without it.
    """
    sleep_s = 10.0
    ...
    try:
        ...
        assert body["job_id"]
        # The documented contract bound, restored by this branch. Kept INSIDE the
        # try so a failure here still drains both jobs in the `finally`.
        assert elapsed < 0.2, (
