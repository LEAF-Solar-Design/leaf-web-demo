...
# ---------------------------------------------------------------------------
# The separator is NOT the hazard; the missing offset is. These three lock that
# distinction, because getting it wrong cost a FALSE LOCK: an earlier revision
# demanded a literal `T`, which would have refused the stamp
# server/routers/jobs.py:359 already produces (`str(health["observed_at"])`, and
# str() of an aware datetime uses a SPACE). A payload the console accepts and
# our own other emitter produces was being downgraded to locked_planned here.
# ---------------------------------------------------------------------------

def _stamp(moment: datetime, separator: str) -> str:
    """`isoformat(sep=" ")` is byte-identical to `str(moment)` for an aware
    datetime, which is exactly how jobs.py builds its stamp."""
    return moment.isoformat(sep=separator)

@pytest.mark.parametrize("separator,spelling", [("T", "literal T"), (" ", "space")])
def test_both_iso_separators_are_accepted_when_the_offset_is_explicit(separator, spelling):
    availability = _shipping("drawing.solve.strings")
    availability["observedAt"] = _stamp(NOW - timedelta(seconds=1), separator)
    availability["expiresAt"] = _stamp(NOW + timedelta(seconds=13), separator)
    availability["evidence"][0]["verifiedAt"] = _stamp(NOW, separator)
    assert is_well_formed_availability(availability, NOW) is True, (
        f"the {spelling} separator with an explicit offset must be accepted")
    # And the console really does accept it, per the port of origin/main above.
    assert _website_validator(availability, NOW) is True, (
        f"the {spelling} separator must be accepted by the console validator too")

def test_the_stamp_jobs_py_already_emits_does_not_false_lock():
    """Reproduces server/routers/jobs.py:359 verbatim, not a synthetic string."""
    health = {"observed_at": NOW - timedelta(seconds=1)}
    observed = str(health["observed_at"])            # jobs.py:359, character for character
    assert observed == "2026-07-24 11:59:59+00:00", observed
    assert " " in observed and "T" not in observed, "str() of an aware datetime uses a SPACE"

    availability = _shipping("drawing.solve.strings")
    availability["observedAt"] = observed
    availability["expiresAt"] = str(NOW + timedelta(seconds=13))
    live = {"drawing.solve.strings": availability}
    descriptors = {d["id"]: d for d in build_descriptors(live, now=NOW)}
    emitted = descriptors["drawing.solve.strings"]["availability"]
    assert emitted["state"] == "shipping", (
        "a space-separated stamp with an offset must survive build_descriptors; "
        f"got a FALSE LOCK: {emitted['state']}")
    assert emitted["observedAt"] == observed, "the accepted stamp is passed through unchanged"

    # The literal spelling the module was written against must still work, and
    # both spellings must land on the SAME instant — accepting the space form and
    # then reading it as a different moment would be worse than rejecting it.
    from product_capability_availability import _parse_iso  # private: same-instant proof
    assert _parse_iso("2026-07-24 12:00:00+00:00") == _parse_iso("2026-07-24T12:00:00+00:00") == NOW

def test_a_missing_offset_is_still_refused_on_either_separator():
    """The safety that mattered, kept: node reads a naive stamp as the viewer's
    LOCAL time while Python reads UTC, so both separators stay rejected without
    an offset. Loosening the separator must not loosen this."""
    for naive in ("2026-07-24 11:59:59", "2026-07-24T11:59:59",
                  "2026-07-24 11:59:59.123", "2026-07-24T11:59:59.123"):
        availability = _shipping("drawing.solve.strings")
        availability["observedAt"] = naive
        assert is_well_formed_availability(availability, NOW) is False, (
            f"{naive!r} carries no offset and must stay rejected")
