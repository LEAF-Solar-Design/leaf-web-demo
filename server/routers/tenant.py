...
carry only ``{linked, linked_at}`` (never the token).
    ...
    POST   /api/tenant/claude-grant  {token}  -> §10 {linked:true, linked_at}
    GET    /api/tenant/claude-grant           -> §10 {linked:bool, linked_at:str|null}
...
def _status_body(harness_json: Dict[str, Any]) -> Dict[str, Any]:
    """Normalize the harness status into a §10-enveloped {linked, linked_at}. Defensive:
    only these two fields are surfaced, so a token can NEVER be echoed even if a future
    harness bug returned one."""
...
def link_grant(req: GrantLinkRequest, tenant=Depends(deps.require_tenant)):
    ...
    try:
        import requests
        r = requests.put(url, json={"token": req.token}, timeout=30)
