...
Literal ports of Branch2025 (read 2026-09-23 at C:/tmp/solar-parity/wt-b25-s17):
  ...
  Terrain/TerrainCommand.cs           PersistElevationGrid (the LEAFTOPO XRecord),
...
network. Floating point work is ordered exactly as the C# orders it (IEEE doubles
...
LEAFTOPO_DICTIONARY_KEY = "LEAFTOPO"              # DictionaryKeys.cs:47
LEAFTOPO_GRID_XRECORD_KEY = "grid"                # TerrainCommand.cs:1727
DXF_REAL = 40                                     # DxfCode.Real, TerrainCommand.cs:1708
DXF_INT32 = 90                                    # DxfCode.Int32, TerrainCommand.cs:1710
DXF_XDATA_INT16 = 1070                            # DxfCode.ExtendedDataInteger16, TrackerSlopeDrawing.cs:289

TOPO_LAYER = "LEAF-TOPO"                          # TerrainCommand.cs:34
TOPO_LAYER_ACI = 8                                # TerrainCommand.cs:1541
...
TRACKER_XDATA_APP = "LEAFTRACKERROW"              # SatCommand.cs:305
FRAME_XDATA_APP = "LEAFFRAMECELL"                 # LeafGenerateCommand.cs:33
VIOLATION_LAYER = "LEAF-SLOPE-VIOLATIONS"         # TrackerSlopeDrawing.cs:12
VIOLATION_LAYER_ACI = 1                           # TrackerSlopeDrawing.cs:13
...
# ---------------------------------------------------------------------------
#  The LEAFTOPO record (TerrainCommand.PersistElevationGrid, :1548-1570, :1681-1741)
# ---------------------------------------------------------------------------

def leaftopo_xrecord(elevations, rows, cols, x_min, x_max, y_min=math.nan, y_max=math.nan,
                     center_lat=0.0, center_lon=0.0, frame=None):
    """The (group code, value) pairs written to NOD["LEAFTOPO"]["grid"], in order.

    :1708-1725: centerLat, centerLon (Real 40, both 0.0 on the 3DFACE path via the
    :1563 overload), rows, cols (Int32 90), every elevation in metres (Real), then
    drawingXMin, drawingXMax, and drawingYMin, drawingYMax only when both are not
    NaN (:1716), then the six-double affine frame only when all six are given
    (AddAffineFrame, :1814-1831). LEAFTOPOFROM3DFACES passes no frame.
    PersistElevationGrid also stamps a new site layout revision (:1740); that write
    is outside this record and is not modelled.
    """
    rows = _int(rows, "rows")
    cols = _int(cols, "cols")
    ...
    values = [(DXF_REAL, _num(center_lat, "center_lat")), (DXF_REAL, _num(center_lon, "center_lon")),
              (DXF_INT32, rows), (DXF_INT32, cols)]
    values.extend((DXF_REAL, _num(e, "elevation")) for e in elevations)
    values.append((DXF_REAL, _num(x_min, "x_min")))
    values.append((DXF_REAL, _num(x_max, "x_max")))
    y_min = _num(y_min, "y_min")
    y_max = _num(y_max, "y_max")
    if not math.isnan(y_min) and not math.isnan(y_max):
        values.append((DXF_REAL, y_min))
        values.append((DXF_REAL, y_max))
    if frame is not None:
        if not isinstance(frame, (list, tuple)) or len(frame) != 6:
            raise TerrainInputError("frame must be six numbers (origin, x axis, y axis)")
        values.extend((DXF_REAL, _num(v, "frame")) for v in frame)
    return values


def _xrecord_values(values):
    if not isinstance(values, (list, tuple)):
        raise TerrainInputError("xrecord values must be a list")
    if len(values) > MAX_GRID_NODES + 16:
        raise TerrainBoundsError("xrecord is larger than any bounded grid")
    out = []
    for i, tv in enumerate(values):
        if isinstance(tv, (list, tuple)) and len(tv) == 2:
            out.append(tv[1])
        else:
            out.append(tv)   # bare values are accepted; only .Value is read
    return out


def _cs_to_int32(value):
    """Convert.ToInt32: integral values pass, doubles round half to even."""
    ...
    raise TerrainInputError(f"cannot convert {value!r} to Int32")


def _cs_to_double(value):
    if isinstance(value, bool) or not isinstance(value, Real):
        raise TerrainInputError(f"cannot convert {value!r} to Double")
    return float(value)


def read_leaftopo_mesh_grid(values):
    """LeafTerrainMeshCommand.TryReadGrid, :102-137. None where the plugin returns
    false (fewer than 4 values, too few for the elevations plus four bounds, or a
    grid under 2x2). Malformed value types raise TerrainInputError (the plugin's
    command catch reports an error rather than drawing)."""
    vals = _xrecord_values(values)
    if len(vals) < 4:
        return None
    rows = _cs_to_int32(vals[2])
    cols = _cs_to_int32(vals[3])
    if rows < 0 or cols < 0 or rows * cols > MAX_GRID_NODES:
        raise TerrainBoundsError(f"grid {rows}x{cols} is out of bounds")
    base = 4 + rows * cols
    if len(vals) < base + 4:
        return None
    grid = {
        "elevations": [_cs_to_double(vals[4 + i]) for i in range(rows * cols)],
        "rows": rows, "cols": cols,
        "x_min": _cs_to_double(vals[base]), "x_max": _cs_to_double(vals[base + 1]),
        "y_min": _cs_to_double(vals[base + 2]), "y_max": _cs_to_double(vals[base + 3]),
    }
    return grid if rows >= 2 and cols >= 2 else None


def read_terrain_interpolator(values, meters_per_unit):
    """TrackerCommand.TryReadTerrainInterpolator, :275-333. None where the plugin
    returns null, including any conversion failure (its catch-all, :332). The
    affine frame is used when six more values follow the Y bounds (:312-326)."""
    if values is None:
        return None
    try:
        vals = _xrecord_values(values)
        ...
        if len(vals) < base + 4:
            return None
        elevations = [_cs_to_double(vals[4 + i]) for i in range(rows * cols)]
        x_min, x_max, y_min, y_max = (_cs_to_double(vals[base + k]) for k in range(4))
        affine = base + 4
        mpu = _cs_to_double(meters_per_unit)
        if len(vals) >= affine + 6:
            frame = [_cs_to_double(vals[affine + k]) for k in range(6)]
            return TerrainGridInterpolator(elevations, rows, cols, x_min, x_max, y_min, y_max,
                                           mpu, *frame)
        return TerrainGridInterpolator(elevations, rows, cols, x_min, x_max, y_min, y_max, mpu)
    except (TerrainInputError, IndexError):
        return None
...
def bucket_color(bucket):
    ...
    The DXF true-colour integer (group 420) is r<<16 | g<<8 | b."""
...
def topo_layer_definition():
    """EnsureTerrainLayer, :1532-1546: LEAF-TOPO created with ACI 8 if missing."""
    return {"name": TOPO_LAYER, "aci": TOPO_LAYER_ACI}
...
def topo_from_3d_faces(faces, meters_per_unit, target_cells=DEFAULT_TARGET_CELLS,
    ...
    "grid", "xrecord" (the LEAFTOPO group-code pairs) and "mesh" (the new faces);
    on a refusal the message is the plugin's and no record or mesh is produced.
    """
    mpu = _meters_per_unit(meters_per_unit)
    target_cells = _int(target_cells, "target_cells")
    ...
    result = {"succeeded": False, "all_faces": len(faces), "source_faces": 0,
              ...
              "grid": None, "xrecord": None, "mesh": None, "cleared_entity_indices": []}
    if not faces:
    ...
    xrecord = leaftopo_xrecord(grid["elevations"], grid["rows"], grid["cols"],
    ...
    result.update(succeeded=True, rows=grid["rows"], cols=grid["cols"], grid=grid,
                  xrecord=xrecord, mesh=mesh, cleared_entity_indices=cleared,
...
def terrain_mesh_rerender(xrecord_values, meters_per_unit, existing_entities=None):
    """LEAFTERRAINMESH, LeafTerrainMeshCommand.cs:50-78: read the stored grid,
    erase LEAF-TOPO, redraw. Returns {"succeeded", "message", "mesh",
    "cleared_entity_indices"}; with no grid the plugin's message and no change."""
    mpu = _meters_per_unit(meters_per_unit)
    grid = read_leaftopo_mesh_grid(xrecord_values) if xrecord_values is not None else None
    if grid is None:
...
def polyline_to_tracker_row(vertices, tracker_xdata, meters_per_unit):
    """TrackerRowReader.PolylineToTrackerRow, :287-337: RowIndex and ModuleSlots
    are xdata values [1] and [2] when at least three are present (:297-301)."""
    if not isinstance(vertices, (list, tuple)) or len(vertices) < 4:
        raise TerrainInputError("tracker polyline needs four vertices")
    p = [_point2(v, "tracker vertex") for v in vertices[:4]]
    mpu = _num(meters_per_unit, "meters_per_unit")
    row_index = module_slots = 0
    vals = _xrecord_values(tracker_xdata or [])
    if len(vals) >= 3:
        row_index = _cs_to_int32(vals[1])
        module_slots = _cs_to_int32(vals[2])
    start = ((p[0][0] + p[1][0]) * 0.5, (p[0][1] + p[1][1]) * 0.5)
    end = ((p[2][0] + p[3][0]) * 0.5, (p[2][1] + p[3][1]) * 0.5)
    return tracker_row(row_index, start, end, module_slots, _distance(start, end) * mpu)


def _frame_cell(xdata):
    """ReadFrameCell, TrackerSlopeDrawing.cs:281-294: first two Int16 (1070) values."""
    row = col = 0
    seen = 0
    for tv in xdata or []:
        if seen >= 2:
            break
        if not isinstance(tv, (list, tuple)) or len(tv) != 2 or tv[0] != DXF_XDATA_INT16:
            continue
        if seen == 0:
            row = _cs_to_int32(tv[1])
        else:
            col = _cs_to_int32(tv[1])
        seen += 1
    return row, col
...
def read_rows_for_slope(entities, preset, meters_per_unit):
    ...
    Entity shapes:
      {"kind": "LWPOLYLINE", "layer", "vertices", "xdata": {app: [(code, value)]}}
        read when on LEAF-TRACKERS with >= 4 vertices (TryReadLeafPolyline,
        :237-279): LEAFTRACKERROW xdata wins, else LEAFFRAMECELL xdata, else skipped.
    ...
    for i, ent in enumerate(entities):
        ...
        if kind == "LWPOLYLINE":
            ...
            if not isinstance(verts, (list, tuple)) or len(verts) < 4:
                continue
            xdata = ent.get("xdata") or {}
            if not isinstance(xdata, dict):
                raise TerrainInputError(f"entities[{i}].xdata must be a dict")
            if xdata.get(TRACKER_XDATA_APP) is not None:
                row = polyline_to_tracker_row(verts, xdata[TRACKER_XDATA_APP], meters_per_unit)
            elif xdata.get(FRAME_XDATA_APP) is not None:
                frame_row, _ = _frame_cell(xdata[FRAME_XDATA_APP])
...
def build_report(leaftopo_values, entities, preset, meters_per_unit):
    """TrackerSlopeDrawing.BuildReport, :63-87. Returns (report or None, message).
    meters_per_unit is what ActiveMetersPerUnit (:15-41) resolved in the drawing;
    that resolution reads drawing state and is the adapter's input here."""
    terrain = read_terrain_interpolator(leaftopo_values, meters_per_unit)
    if terrain is None:
...
def violation_layer_definition():
    """EnsureLayer, TrackerSlopeDrawing.cs:296-308: LEAF-SLOPE-VIOLATIONS, ACI 1."""
    return {"name": VIOLATION_LAYER, "aci": VIOLATION_LAYER_ACI}
...
def tracker_slope_violations(leaftopo_values, entities, preset, meters_per_unit):
    ...
    report, message = build_report(leaftopo_values, entities, preset, meters_per_unit)
    if report is None:
