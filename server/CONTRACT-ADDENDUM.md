...
| Method | Path | Behaviour |
|---|---|---|
| POST | `/broker/run` | `{tenant_id, tool, params, dwg, aps_live, dwg_version?, test_source?}` → extended §3 envelope. `aps_live:false` → pure-python mock path; `true` → `da.client.run_tool` (live). `dwg_version` pins the §11 store version (live reads reject the pin fail-closed until wired). Additive `test_source` is accepted only for a design-time non-live test, runs only in the configured sandbox, and contributes only its SHA-256 digest to request identity and ledgers. |
