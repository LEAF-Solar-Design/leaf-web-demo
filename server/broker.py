...
def _resolve_reap_workitems(records: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    the id up by job_id. Resolved entries are dropped from the registry so the
    same WorkItem is not cancelled twice. Records that already carry a
    workitem_id, or that name no known job, pass through untouched (sweep leaves
    an id-less orphan's cancel as a no-op, exactly as before).
    """
    out: List[Dict[str, Any]] = []
    for rec in records:
        rec = dict(rec)
        if not rec.get("workitem_id"):
            resolved = _drop_active_workitem(rec.get("job_id"))
            if resolved:
    ...
    if req.aps_live:
        ...
        if da is None or not hasattr(da, "run_tool"):
            ...
            try:
                ...
                on_submitted = None
                if req.job_id:
                    def on_submitted(workitem_id, _job_id=req.job_id):
                        _record_active_workitem(_job_id, workitem_id)
