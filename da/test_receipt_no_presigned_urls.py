#!/usr/bin/env python3
"""Ban test: no committed receipt under data/ may carry a presigned QUERY STRING.
...
`X-Amz-Expires` seconds. da/ drivers write that value into JSON receipts under
data/, and those receipts are COMMITTED. This repository went PUBLIC on
2026-08-24, so a receipt generated and committed inside the credential's
lifetime publishes a working credentialed url. GitHub secret scanning opened
six `aws_temporary_access_key_id` alerts on exactly that.

WHY IT ASSERTS ON QUERY PARAMETERS, NOT URL SHAPE
-------------------------------------------------
A shape assertion ("the url must look like `.../report?<redacted-presigned>`")
is WORSE THAN NOTHING: it passes any url that does not match the redacted
shape, including a LIVE credentialed one, and it breaks the moment a driver
emits a legitimately different url. So the ban is stated over the thing that
actually leaks - the presigned query parameters in
`redact.PRESIGNED_QUERY_PARAMS` - and it is stated over the RAW file text, so
a credential hidden in a key, a nested string, an error message, or a field
nobody has invented yet is caught the same way a `reportUrl` is.
...
from redact import PRESIGNED_QUERY_PARAMS, redact_report_url  # noqa: E402
...
# evidence. These two run FIRST and fail loudly if the scanner is a no-op.
# --------------------------------------------------------------------------- #
...
def test_scanner_accepts_the_redacted_form():
    """Redaction must actually satisfy the ban - otherwise the fix and the test
    disagree and the gate can never go green."""
    assert not _presigned_params_in(redact_report_url(SAMPLE_PRESIGNED))
...
def test_data_dir_exists_and_holds_json():
    ...
    assert _receipt_paths(), f"no *.json found under {DATA_DIR}; this ban scans nothing"


@pytest.mark.parametrize("name", KNOWN_RECEIPTS)
def test_known_receipts_are_actually_scanned(name):
    scanned = {os.path.basename(p) for p in _receipt_paths()}
...
# The ban itself
# --------------------------------------------------------------------------- #
@pytest.mark.parametrize("path", _receipt_paths(),
...
def test_committed_json_carries_no_presigned_query_parameter(path):
    ...
    assert not hits, (
        ...
        f"{hits}. That is a live AWS credential published in a PUBLIC repo. "
        "Route the value through da/redact.redact_report_url() AT WRITE TIME in "
        "the generator that produced this file - do not hand-edit the receipt "
        "and leave the generator emitting credentials."
    )
...
def test_committed_json_urls_carry_no_query_credentials(path):
    ...
    by its actual query parameters. Catches a credential that survived the raw
    scan through unusual encoding, and names the exact field when it fires."""
    try:
        ...
        if isinstance(node, dict):
            ...
            for chunk in node.split():
                if "://" not in chunk or "?" not in chunk:
                    continue
                query = urlparse(chunk).query
                banned = {p for p in PRESIGNED_QUERY_PARAMS
                          if p.lower() in {k.lower() for k in parse_qs(query)}}
                if banned:
                    offenders.append((where, sorted(banned)))
