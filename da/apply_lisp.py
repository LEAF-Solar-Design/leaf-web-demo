...
_LISP_LINES = (
    ...
    '(defun leaf-setpoints-preserve (op data / item code pts out saw38 saw210) (setq pts (nth 5 op)) (foreach item data (setq code (car item)) (cond ((= code 10) (setq out (cons (cons 10 (car pts)) out) pts (cdr pts))) ((= code 70) (setq out (cons (cons 70 (nth 2 op)) out))) ((= code 38) (setq out (cons (cons 38 (nth 4 op)) out) saw38 T)) ((= code 210) (setq out (cons (cons 210 (nth 3 op)) out) saw210 T)) (T (setq out (cons item out))))) (setq out (reverse out)) (if (not saw38) (setq out (append out (list (cons 38 (nth 4 op)))))) (if (not saw210) (setq out (append out (list (cons 210 (nth 3 op)))))) out)',
    '(defun leaf-apply-setpoints (op / e data) (setq e (handent (nth 1 op)) data (entget e)) (if (and (= (cdr (assoc 0 data)) "LWPOLYLINE") (= (length (nth 5 op)) (leaf-count-code data 10))) (if (entmod (leaf-setpoints-preserve op data)) (progn (entupd e) T)) (leaf-apply-setpoints-rebuild op)))',
    '(defun leaf-apply-setpoints-rebuild (op / e data kind item code head saw38 saw90 saw70 n pts normal elev closed out) (setq e (handent (nth 1 op)) data (entget e) kind (cdr (assoc 0 data)) closed (nth 2 op) normal (nth 3 op) elev (nth 4 op) pts (nth 5 op) n (length pts)) (cond ((= kind "LINE") (if (and (= n 2) (= closed 0)) (progn (setq out (subst (cons 10 (trans (list (car (car pts)) (cadr (car pts)) elev) normal 0)) (assoc 10 data) data)) (setq out (subst (cons 11 (trans (list (car (cadr pts)) (cadr (cadr pts)) elev) normal 0)) (assoc 11 out) out)) (if (entmod out) (progn (entupd e) T))))) ((= kind "LWPOLYLINE") (foreach item data (setq code (car item)) (cond ((member code (list 10 40 41 42 210)) nil) ((= code 90) (setq head (cons (cons 90 n) head) saw90 T)) ((= code 70) (setq head (cons (cons 70 closed) head) saw70 T)) ((= code 38) (setq head (cons (cons 38 elev) head) saw38 T)) (T (setq head (cons item head))))) (setq head (reverse head)) (if (not saw90) (setq head (append head (list (cons 90 n))))) (if (not saw70) (setq head (append head (list (cons 70 closed))))) (if (not saw38) (setq head (append head (list (cons 38 elev))))) (foreach p pts (setq head (append head (list (cons 10 p))))) (setq head (append head (list (cons 210 normal)))) (if (entmod head) (progn (entupd e) T)))))',
