...
_MLEADER_LISP_LINES = (
    ...
    '(defun leaf-apply-addmleader (op / layer style p1 p2 text oldlayer oldstyle before result caught layerdata) (setq layer (nth 1 op) style (nth 2 op) p1 (nth 3 op) p2 (nth 4 op) text (nth 5 op) layerdata (tblsearch "LAYER" layer)) (if (and (leaf-mlstyle style) (not (and layerdata (= 1 (logand 1 (cdr (assoc 70 layerdata))))))) (progn (setq oldlayer (getvar "CLAYER") oldstyle (getvar "CMLEADERSTYLE")) (setq caught (vl-catch-all-apply (function (lambda () (leaf-ensure-layer layer) (setvar "CLAYER" layer) (setvar "CMLEADERSTYLE" style) (setq before (entlast)) (setq p1 (trans p1 0 1) p2 (trans p2 0 1)) (command "_.MLEADER" p1 p2 text))) nil)) (setvar "CLAYER" oldlayer) (setvar "CMLEADERSTYLE" oldstyle) (setq result (entlast)) (if (and (not (vl-catch-all-error-p caught)) result (not (equal result before)) (= (cdr (assoc 0 (entget result))) "MULTILEADER")) result))))',
)
