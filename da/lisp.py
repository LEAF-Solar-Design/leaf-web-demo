...
MUTATION_INSPECT_BLOCKS += (
    ...
    '(defun leaf-ml-row (data / state closed branches vertices height arrow textpt text attachment landing doglegdir dogleg style textstyle content) (setq state 0 branches 0 vertices "") (leaf-ml-walk data) (if (and (= branches 1) (= content 2) text (/= vertices "") landing) (write-line (strcat "ML|" (cdr (assoc 5 data)) "|" (leaf-bk-encode (cdr (assoc 8 data))) "|" (leaf-bk-encode (cond (style) (T ""))) "|" (leaf-bk-encode (cond (textstyle) (T ""))) "|" (rtos height 2 5) "|" (rtos arrow 2 5) "|" (rtos dogleg 2 5) "|" (itoa attachment) "|" vertices "|" (leaf-bk-point landing 5) "|" (leaf-bk-point doglegdir 5) "|" (leaf-bk-point textpt 5) "|" (leaf-bk-encode text)) f) (write-line "MLX|1" f)))',
