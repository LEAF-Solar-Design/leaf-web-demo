...
| --- | --- | --- | --- |
| `ground` | `'drawing' \| 'board' \| 'device-stage' \| 'sheet'` | the canvas kind under the shell | `web/src/site/SurfaceGrounds.jsx:111` derives `DRAWING_SURFACES` from the contract (`contract.ground === 'drawing'`), read by `groundShowsDrawing()` at `:116`; it replaced the literal `new Set(['cad','solar'])`; the two non-drawing kinds are named in `web/e2e/local/one-shell-mount.spec.mjs:131` and rendered by `ProjectBoardGround` (`SurfaceGrounds.jsx:133`, active on `surfaceGround(surface) === 'board'` at `:298`, was `surface === 'browser'`) and `DeviceGround` (`:222`, active on `=== 'device-stage'` at `:306`, was `surface === 'ios'`) |
