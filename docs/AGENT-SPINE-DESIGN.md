...
- **`undo_drawing_version` is policy `auto` even though it is a write** — a deliberate safety valve. Undo must never be harder than the write it reverses. It dispatches to the existing repoint-head route (`server/routers/drawings.py:60-67`); objects are never deleted. **REQUIRED CONTRACT ADDENDUM (blocker until resolved)**: `POST /api/drawings/{id}/undo` is **not** on the pinned wire contract's back-edge `X-Dispatch-Secret` allowlist (section 0 / §18.5 accepts POST only on `/api/run` and `/internal/agent/gate`; `/api/drawings/*` is GET-only), and none of the six spine tools dispatches it (`run_capability` only dispatches `POST /api/run`) — as pinned, a harness-dispatched undo 401s and this Phase-1-LIVE action has no invocation path. Before any lane builds against this action, the contract owner must (a) add `POST /api/drawings/{id}/undo` to the section-0 allowlist AND (b) pin the invoking spine tool (`run_capability` recognizing the reserved `undo_drawing_version` action name). Per the contract preamble, this is a STOP-and-report deviation, not a silent route extension; the catalog entry below records the *intended* route pending that addendum.
...
```json
{
  ...
  "actions": [
    ...
    },
    {
      ...
      "description": "Repoint drawing head to the previous version (safety valve: never harder than the write).",
      "rung": "R3",
      "required_capability": "run_write",
      "policy": "auto",
      "dispatch": { "kind": "app_api", "routes": ["POST /api/drawings/{id}/undo"] },
      "args_schema": { "type": "object",
...
Two of these are load-bearing for safety: confirmation chips render **server truth** from the pending-approval record, never model prose (anti-spoof, §5), and undo must never be harder than the write it reverses (`undo_drawing_version` is policy `auto` — §5 ladder note; undo verified live in `data/write_loop_receipt.json`, `undo_verified: true`, measured — but note the harness currently has **no invocation path** for it: see the §8.4 contract-gap item and the §5.1 R3 blocker note).
...
**Phase-1 contract gap (carried from §5.1, blocker until resolved)**: `undo_drawing_version` is Phase-1-LIVE and policy-auto, but it has **no harness invocation path** — `POST /api/drawings/{id}/undo` is not on the wire-contract §0 back-edge allowlist and no spine tool dispatches it, so a harness-dispatched undo 401s today. The contract owner must add the route to the §0 allowlist and pin the invoking tool (see the §5.1 R3 note). Per the contract preamble this is a STOP-and-report deviation, and it blocks Phase-1 completion of the R3 undo story.
