...
tenant's authored tool must be absent from the catalog. Direct reads of the
other staged change, publication approval, drawing, and job must return only
403 or 404. Forged tenant headers must not change the caller's audit
projection. Every 2xx response for a direct cross-tenant read fails.
